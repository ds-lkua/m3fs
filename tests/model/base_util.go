...
// TeardownEnv teardown test environment about database.
func TeardownEnv() (err error) {
	if defaultDB == nil {
		return nil
	}

	defer func() {
		if e := closeDB(defaultDB); e != nil {
			log.Logger.Errorf("Failed to close db: %s", e)
		}
	}()

	err = defaultDB.Exec("DROP DATABASE ?", clause.Table{Name: dbTemplate}).Error
	if err != nil {
		return errors.Annotatef(err, "drop database %s", dbTemplate)
	}
	log.Logger.Infof("Template database %s deleted", dbTemplate)

	return nil
}

// TestMain test main function for all model based testing program.
